...
class UTrackingAnimator():
    ...
    def plot_static(self, frame):
        ...
        fig = make_subplots(rows=2, cols=2,
                            specs=[[{"rowspan": 2}, {}],
                                   [None, {}]],
                            subplot_titles=("Episode", "Reward", "Prediction error"))
        ...
        for l in range(self.n_landmarks): 
            ...
            # predictions head
            fig.add_trace(go.Scatter(x=self.landmark_predictions[l, frame-1:frame, 0],
                                     y=self.landmark_predictions[l, frame-1:frame, 1],
                                     mode='markers',
                                     marker_symbol='pentagon',
                                     marker=dict(size=7, opacity=0.4, color=px.colors.sequential.Pinkyl[::-1][l]),
                                     name=f'Landmark {l+1} predicted'),
                         row=1, col=1) 

             # landmark predicted
            fig.add_trace(go.Scatter(x=self.landmark_predictions[l, :frame, 0],
                                     y=self.landmark_predictions[l, :frame, 1],
                                     mode='markers',
                                     marker_symbol='pentagon',
                                     showlegend=False,
                                     marker=dict(size=7, opacity=0.2, color=px.colors.sequential.Pinkyl[::-1][l])),
                         row=1, col=1)

        # landmark errors
        for l in range(self.n_landmarks): 
            fig.add_trace(go.Scatter(x=np.arange(frame),
                                        y=self.episode_errors[l, :frame],
                                        mode='lines',
                                        marker=dict(color=px.colors.sequential.Pinkyl[::-1][l]),
                                        name=f'Landmark {l+1} error'),
                            row=2, col=2)
