...
def assert_is_valid_key(key):
    ...
    if (
        ...
        and (key in sacred.jsonpickle.tags.RESERVED or key.startswith("json://"))
    ):
        raise KeyError(
            'Invalid key "{}". Config-keys cannot be one of the'
            "reserved jsonpickle tags: {}".format(key, sacred.jsonpickle.tags.RESERVED)
        )
