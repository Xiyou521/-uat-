...
import numpy as np
from utracking.multi_discrete import MultiDiscrete
import time
...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(
        ...
        observation_full_callback=None, 
        info_callback=None,
        done_callback=None, 
        shared_viewer=True
    ):
        ...
        for agent in self.agents:
            ...
            if self.discrete_action_space:
                # three discrete actions: still(0), left(-1), right(+1)
        ...
        # agents: pos(2), vel(2), landmarks: pos(2), vel(2), depth(1)
        self.state_space = (world.num_agents + world.num_landmarks)*6
    ...
    def step(self, action_n):
        ...
        # advance world state
        self.world.step()
        # record observation for each agent (separete step since some parameters are updated here)
        obs_n = [self._get_obs(agent) for agent in self.agents]
        # get the world reward, info and done
        ...
        for i in range(self.world.num_landmarks):
            self.world.landmarks_best_estimations[i].size = self.world.cov[i] 
    ...
    def reset(self):
        ...
        obs_n = [self._get_obs(agent) for agent in self.world.policy_agents]    
        return obs_n
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                ...
                if action[0] == 1: agent.action.u = -1.0
                if action[0] == 2: agent.action.u = +1.0
            else:
    ...
    def render(self, mode='human'):
        ...
        for i in range(len(self.viewers)):
            ...
            if self.viewers[i] is None:
                ...
                from utracking import rendering
